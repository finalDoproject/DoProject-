...
public class ChatCountController {
	...
	public ResponseBodyEmitter handleRequest(HttpSession session) {
		...
		for(Member me : list) {
			for(Member you : list) {
				if(me != you) {
					System.out.println(me.toString() + " : " + you.toString());
				}
			}
		}
			...
			for(int i = 0; i < 500; i++) {
				try {
					...
					for(Member me : list) {
						for(Member you : list) {
							if(me != you) {
								mtm = chatService.selectOneChatMtm(me.getNickName(), p.getPno(), me.getMno(), you.getMno());
								emitter.send("m"+me.getMno()+"TOm"+you.getMno()+":"+mtm, MediaType.TEXT_PLAIN);
							}
						}
					}
